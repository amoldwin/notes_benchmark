...
class Discretizer:
    def __init__(self, timestep=0.8, store_masks=True, impute_strategy='zero', start_time='zero',
                 config_path=os.path.join(os.path.dirname(__file__), 'resources/discretizer_config.json'), header=None, sources=None):
        assert not header==None
        ...
        self.structured_channels = self._id_to_channel
        self._id_to_channel = self._header[1:]
        self._channel_to_id = dict(zip(self._id_to_channel, range(len(self._id_to_channel))))
        self._is_categorical_channel = config['is_categorical_channel']
        for channel in [channel for channel in self._channel_to_id if channel not in self.structured_channels]:
            self._is_categorical_channel[channel]=False
            self._possible_values[channel] = []
            self._normal_values[channel] = 0.0
