...
def read_events_table_by_row(mimic3_path, table):
    print('table was',table)
    nb_rows = {'chartevents': 330712484, 'labevents': 27854056, 'outputevents': 4349219, 'noteevents':2083179 , 'wordevents':2083179 , 'cuievents':2083179, 'doc2vec':2083179 }
    if table == 'wordevents':
        reader = csv.DictReader(open(os.path.join('../../LHC_mimic/mimic3_1.4/derived/notes.words.csv'), 'r'),fieldnames = ( 'SUBJECT_ID', 'HADM_ID', 'CHARTDATE', 'CHARTTIME', 'CATEGORY', 'DESCRIPTION', 'WORDS'))
    elif table == 'cuievents':
        reader = csv.DictReader(open(os.path.join('../../LHC_mimic/mimic3_1.4/derived/notes.cuis.csv'), 'r'),fieldnames = ( 'SUBJECT_ID', 'HADM_ID', 'CHARTDATE', 'CHARTTIME', 'CATEGORY', 'DESCRIPTION', 'CUIS'))
    elif table == 'doc2vec':
        reader = csv.DictReader(open(os.path.join(mimic3_path, 'NOTEEVENTS' + '.csv'), 'r'))    
    else:
...
def read_events_table_and_break_up_by_subject(mimic3_path, table, output_path, items_to_keep=None, subjects_to_keep=None, verbose=1):
    ...
    def write_current_observations():
        ...
        data_stats.curr_obs = []
    if table == 'doc2vec':
        #load doc2vec model trained on all mimic notes
        doc2vec_model_path = 'resources/doc2vec_allnotes.pk'
        X = pickle.load(open(doc2vec_model_path, "rb")) 
        print('Loaded Doc2vec')
    def create_doc2vec_encoded_text(vectorizer,text):
        return vectorizer.infer_vector(word_tokenize((text))).tolist()
    ...
    for row, row_no, nb_rows in read_events_table_by_row(mimic3_path, table):
        ...
        if (subjects_to_keep is not None) and (row['SUBJECT_ID'] not in subjects_to_keep):
            continue
        if (items_to_keep is not None) and (row['ITEMID'] not in items_to_keep) and 'TEXT' not in row:
            continue
        if row['CHARTTIME']=='':
            continue
        ...
        row_out = {'SUBJECT_ID': row['SUBJECT_ID'],
                   ...
                   'ITEMID': '-1' if table=='NOTEEVENTS' else '-2' if table=='cuievents' else '-3' if table=='wordevents' else '-4' if table=='doc2vec' else row['ITEMID'],
                   'VALUE': row['TEXT'] if table=='NOTEEVENTS' else row['WORDS'] if table=='wordevents' else row['CUIS'] if table=='cuievents' else create_doc2vec_encoded_text(X,row['TEXT']) if table=='doc2vec' else row['VALUE'],
                   'VALUEUOM': 'TEXT' if table=='NOTEEVENTS' else 'CUIS' if 'CUIS' in row else 'WORDS' if 'WORDS' in row else 'doc2vec' if table=='doc2vec' else row['VALUEUOM']}
