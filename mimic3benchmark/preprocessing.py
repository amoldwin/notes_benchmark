...
def read_itemid_to_variable_map(fn, variable_column='LEVEL2'):
    ...
    emptymap1 = pd.DataFrame({"ITEMID":[-1], "LEVEL2": ["TEXT"], "MIMIC LABEL":["TEXT"]})
    emptymap2 = pd.DataFrame({"ITEMID":[-2], "LEVEL2": ["CUIS"], "MIMIC LABEL":["CUIS"]})
    emptymap3 = pd.DataFrame({"ITEMID":[-3], "LEVEL2": ["WORDS"], "MIMIC LABEL":["WORDS"]})
    emptymap4 = pd.DataFrame({"ITEMID":[-4], "LEVEL2": ["DOC2VEC"], "MIMIC LABEL":["DOC2VEC"]})
    ...
    var_map = var_map.append(emptymap1)
    var_map = var_map.append(emptymap2)
    var_map = var_map.append(emptymap3)
    var_map = var_map.append(emptymap4)
...
def read_variable_ranges(fn, variable_column='LEVEL2'):
    ...
    return var_ranges[var_ranges.notnull().all(axis=1)]
...
def remove_outliers_for_variable(events, variable, ranges):
    ...
    V = events.VALUE[idx]
    V[V < ranges.OUTLIER_LOW[variable]] = np.nan
    V[V > ranges.OUTLIER_HIGH[variable]] = np.nan
    V[V < ranges.VALID_LOW[variable]] = ranges.VALID_LOW[variable]
    V[V > ranges.VALID_HIGH[variable]] = ranges.VALID_HIGH[variable]
    events[idx, 'VALUE'] = V
    return events
