...
for subject_dir in os.listdir(args.subjects_root_path):
    ...
    for i in range(stays.shape[0]):
        ...
        episodic_data.Weight[stay_id] = get_first_valid_from_timeseries(episode, 'Weight')
        episodic_data.Height[stay_id] = get_first_valid_from_timeseries(episode, 'Height')
        episodic_data[episodic_data.index==stay_id].to_csv(os.path.join(args.subjects_root_path, subject_dir, 'episode{}.csv'.format(i+1)), index_label='Icustay')
