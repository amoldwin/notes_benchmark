...
def read_noteevents_table(mimic3_path):
    notes = dataframe_from_csv(os.path.join(mimic3_path, 'NOTEEVENTS.csv'))
    notes = notes[['SUBJECT_ID', 'HADM_ID','CHARTDATE', 'CHARTTIME']]
...
def read_events_table_by_row(mimic3_path, table):
    nb_rows = {'chartevents': 330712484, 'labevents': 27854056, 'outputevents': 4349219, 'noteevents':4349219}
...
def read_events_table_and_break_up_by_subject(mimic3_path, table, output_path, items_to_keep=None, subjects_to_keep=None, verbose=1):
    ...
    for row, row_no, nb_rows in read_events_table_by_row(mimic3_path, table):
        ...
        row_out = {'SUBJECT_ID': row['SUBJECT_ID'],
                   ...
                   'ITEMID': '-1' if 'TEXT' in row else row['ITEMID'],
                   'VALUE': row['TEXT'] if 'TEXT' in row else row['VALUE'],
                   'VALUEUOM': 'ROW_ID' if 'TEXT' in row else row['VALUEUOM']
